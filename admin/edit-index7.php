...
for ($i= 0; $i < count($promise7Array); $i++ ) {
    ...
    $sqlx = "UPDATE presidents SET index7='{$index7}', promise7='{$promise7}', signal7='{$signal7}', doings7='{$doings7}' WHERE id='{$identity}' ";
        ...
        $msg_good = 'President profile is edited now, <a href=edit-president.php?mixhtyr12noitcudorpobguhtennek007='.$id.'>Click here to continue</a>';
        }
    }
        ...
        if($res){       
            ...
            $row = $results->fetch_assoc();
                ...
                if ($allColumnsNotEmpty) {
                    ...
                    if($resp){
                        $msg_good = "President profile is now completely editted";
                    }
                }else{
                    $msg_good = 'President profile is edited now but not completed, <a href=edit-president.php?mixhtyr12noitcudorpobguhtennek007='.$id.'>Click here to continue</a>';
                }
                
            }else{
                $msg_bad = "Unable to edit President profile";
            }
...
echo
        ...
        <div id="filiv" class="form-group">
            ...
            <textarea class="form-control" rows="5" name="doings7[]">'.$doin7[$i].'</textarea>  
        </div>'; 
