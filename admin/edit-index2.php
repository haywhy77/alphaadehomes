...
$index2Array = $_POST['index2'];
$promise2Array= $_POST['promise2'];
$signal2Array = $_POST['signal2'];
$doings2Array = $_POST['doings2'];



for ($i= 0; $i < count($promise2Array); $i++ ) {

    $index2  = mysqli_real_escape_string($conn, $index2Array);  
    $promise2  = mysqli_real_escape_string($conn, implode('^', $promise2Array)); 
    $signal2  = mysqli_real_escape_string($conn, implode('^', $signal2Array));    
    $doings2  = mysqli_real_escape_string($conn, implode('^', $doings2Array));       




    $sqlx = "UPDATE presidents SET index2='{$index2}', promise2='{$promise2}', signal2='{$signal2}', doings2='{$doings2}' WHERE id='{$identity}' ";
 ...
 echo '<script type="text/javascript">window.location.replace("edit-index3.php?htennek007='.$identity.'");</script>';
               }
...
<label>'.$index2.'</label>
<input type="hidden" class="form-control"   id="index1"  name="index2" value="'.$index2.'">  
</div>';  


$prom1 = explode("^",$promise2); 
$doin1 = explode("^", $doings2);
$sig1 = explode("^", $signal2);
$j = 0;
...
echo
        '<div class="form-group">  
            ...
            <textarea name="promise2[]" id="txt" rows="5" value="'.$prom1[$i].'" class="form-control">'.$prom1[$i].'</textarea>
        </div> 

        <div class="form-group">
            <label>Performance Signal</label>
            <select name="signal2[]" class="form-control">
        ...
        <div id="filiv" class="form-group">
            ...
            <textarea class="form-control" rows="5" name="doings2[]">'.$doin1[$i].'</textarea>  
        </div>'; 
    ...
    <script type="text/javascript">
        $("#rowAdder").click(function () {
            newRowAdd =
                ...
                '<textarea name="promise2[]" rows="5" class="form-control m-input"></textarea></div>'+
                '<label>Performance Signal</label><div class="input-group"><select name="signal2[]" class="form-control m-input">'+
                '<option value="Invalid Selection">-Select Performance-</option>'+
                '<option value="High Performance">High Performance</option>'+
                '<option value="Medium Performance">Medium Performance</option>'+
                '<option value="Low Performance">Low Performance</option>'+
                '</select></div>'+
                '<label>Actual Doings</label><div class="input-group"><textarea class="form-control m-input" rows="5" name="doings2[]"></textarea></div> </div> </div>';
