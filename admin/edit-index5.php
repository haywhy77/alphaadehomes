...
$index5Array = $_POST['index5'];
$promise5Array= $_POST['promise5'];
$signal5Array = $_POST['signal5'];
$doings5Array = $_POST['doings5'];



for ($i= 0; $i < count($promise5Array); $i++ ) {

    $index5  = mysqli_real_escape_string($conn, $index5Array);  
    $promise5  = mysqli_real_escape_string($conn, implode('^', $promise5Array)); 
    $signal5  = mysqli_real_escape_string($conn, implode('^', $signal5Array));    
    $doings5  = mysqli_real_escape_string($conn, implode('^', $doings5Array));       




    $sqlx = "UPDATE presidents SET index5='{$index5}', promise5='{$promise5}', signal5='{$signal5}', doings5='{$doings5}' WHERE id='{$identity}' ";
 ...
 echo '<script type="text/javascript">window.location.replace("edit-index6.php?htennek007='.$identity.'");</script>';
               }
...
<label>'.$index5.'</label>
<input type="hidden" class="form-control"   id="index1"  name="index5" value="'.$index5.'">  
</div>';  


$prom1 = explode("^",$promise5); 
$doin1 = explode("^", $doings5);
$sig1 = explode("^", $signal5);
$j = 0;
...
echo
        '<div class="form-group">  
            ...
            <textarea name="promise5[]" id="txt" rows="5" value="'.$prom1[$i].'" class="form-control">'.$prom1[$i].'</textarea>
        </div> 

        <div class="form-group">
            <label>Performance Signal</label>
            <select name="signal5[]" class="form-control">
        ...
        <div id="filiv" class="form-group">
            ...
            <textarea class="form-control" rows="5" name="doings5[]">'.$doin1[$i].'</textarea>  
        </div>'; 
    ...
    <script type="text/javascript">
        $("#rowAdder").click(function () {
            newRowAdd =
                ...
                '<textarea name="promise5[]" rows="5" class="form-control m-input"></textarea></div>'+
                '<label>Performance Signal</label><div class="input-group"><select name="signal5[]" class="form-control m-input">'+
                '<option value="Invalid Selection">-Select Performance-</option>'+
                '<option value="High Performance">High Performance</option>'+
                '<option value="Medium Performance">Medium Performance</option>'+
                '<option value="Low Performance">Low Performance</option>'+
                '</select></div>'+
                '<label>Actual Doings</label><div class="input-group"><textarea class="form-control m-input" rows="5" name="doings5[]"></textarea></div> </div> </div>';
