...
$index6Array = $_POST['index6'];
$promise6Array= $_POST['promise6'];
$signal6Array = $_POST['signal6'];
$doings6Array = $_POST['doings6'];



for ($i= 0; $i < count($promise6Array); $i++ ) {

    $index6  = mysqli_real_escape_string($conn, $index6Array);  
    $promise6  = mysqli_real_escape_string($conn, implode('^', $promise6Array)); 
    $signal6  = mysqli_real_escape_string($conn, implode('^', $signal6Array));    
    $doings6  = mysqli_real_escape_string($conn, implode('^', $doings6Array));       




    $sqlx = "UPDATE presidents SET index6='{$index6}', promise6='{$promise6}', signal6='{$signal6}', doings6='{$doings6}' WHERE id='{$identity}' ";
 ...
 echo '<script type="text/javascript">window.location.replace("edit-index7.php?htennek007='.$identity.'");</script>';
               }
...
<label>'.$index6.'</label>
<input type="hidden" class="form-control"   id="index1"  name="index6" value="'.$index6.'">  
</div>';  


$prom1 = explode("^",$promise6); 
$doin1 = explode("^", $doings6);
$sig1 = explode("^", $signal6);
$j = 0;
...
echo
        '<div class="form-group">  
            ...
            <textarea name="promise6[]" id="txt" rows="5" value="'.$prom1[$i].'" class="form-control">'.$prom1[$i].'</textarea>
        </div> 

        <div class="form-group">
            <label>Performance Signal</label>
            <select name="signal6[]" class="form-control">
        ...
        <div id="filiv" class="form-group">
            ...
            <textarea class="form-control" rows="5" name="doings6[]">'.$doin1[$i].'</textarea>  
        </div>'; 
    ...
    <script type="text/javascript">
        $("#rowAdder").click(function () {
            newRowAdd =
                ...
                '<textarea name="promise6[]" rows="5" class="form-control m-input"></textarea></div>'+
                '<label>Performance Signal</label><div class="input-group"><select name="signal6[]" class="form-control m-input">'+
                '<option value="Invalid Selection">-Select Performance-</option>'+
                '<option value="High Performance">High Performance</option>'+
                '<option value="Medium Performance">Medium Performance</option>'+
                '<option value="Low Performance">Low Performance</option>'+
                '</select></div>'+
                '<label>Actual Doings</label><div class="input-group"><textarea class="form-control m-input" rows="5" name="doings6[]"></textarea></div> </div> </div>';
