...
$index4Array = $_POST['index4'];
$promise4Array= $_POST['promise4'];
$signal4Array = $_POST['signal4'];
$doings4Array = $_POST['doings4'];



for ($i= 0; $i < count($promise4Array); $i++ ) {

    $index4  = mysqli_real_escape_string($conn, $index4Array);  
    $promise4  = mysqli_real_escape_string($conn, implode('^', $promise4Array)); 
    $signal4  = mysqli_real_escape_string($conn, implode('^', $signal4Array));    
    $doings4  = mysqli_real_escape_string($conn, implode('^', $doings4Array));       




    $sqlx = "UPDATE presidents SET index4='{$index4}', promise4='{$promise4}', signal4='{$signal4}', doings4='{$doings4}' WHERE id='{$identity}' ";
 ...
 echo '<script type="text/javascript">window.location.replace("edit-index5.php?htennek007='.$identity.'");</script>';
               }
...
<label>'.$index4.'</label>
<input type="hidden" class="form-control"   id="index1"  name="index4" value="'.$index4.'">  
</div>';  


$prom1 = explode("^",$promise4); 
$doin1 = explode("^", $doings4);
$sig1 = explode("^", $signal4);
$j = 0;
...
echo
        '<div class="form-group">  
            ...
            <textarea name="promise4[]" id="txt" rows="5" value="'.$prom1[$i].'" class="form-control">'.$prom1[$i].'</textarea>
        </div> 

        <div class="form-group">
            <label>Performance Signal</label>
            <select name="signal4[]" class="form-control">
        ...
        <div id="filiv" class="form-group">
            ...
            <textarea class="form-control" rows="5" name="doings4[]">'.$doin1[$i].'</textarea>  
        </div>'; 
    ...
    <script type="text/javascript">
        $("#rowAdder").click(function () {
            newRowAdd =
                ...
                '<textarea name="promise4[]" rows="5" class="form-control m-input"></textarea></div>'+
                '<label>Performance Signal</label><div class="input-group"><select name="signal4[]" class="form-control m-input">'+
                '<option value="Invalid Selection">-Select Performance-</option>'+
                '<option value="High Performance">High Performance</option>'+
                '<option value="Medium Performance">Medium Performance</option>'+
                '<option value="Low Performance">Low Performance</option>'+
                '</select></div>'+
                '<label>Actual Doings</label><div class="input-group"><textarea class="form-control m-input" rows="5" name="doings4[]"></textarea></div> </div> </div>';
