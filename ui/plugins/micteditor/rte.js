 ...
 * MictEditor v1.0.0
 * License - FREE
 * Copyright 2024 Adedeji Richards.
 */
// if (!window.MictEditor) window.MictEditor = { };
const MictEditor = (() => {
    ...
    function generateId(min, max) { // min and max included 
    ...
    function init(container, config = {}) {
        ...
        const id =container.getAttribute('id') || generateId(1, 100);;
        editor = document.createElement('div');
        editor.id = "mt-editor_"+id;
        editor.className = 'mt-editor';
        editor.contentEditable = true;
        editor.innerHTML=container.innerHTML;
        window.editor=this;
        // console.log("Container: ", container);
        // console.log("Default content: ", container.innerHTML);
        container.innerHTML=""
        const imageToolbar = document.createElement('div');
        imageToolbar.className = 'mt-image-toolbar';
        ...
        const handleFileUpload = (event, handler) => {
            ...
            if (handler) {
                ...
                reader.onload = (e) => {
                    ...
                    media.className = `mt-editor-${event.target.accept.startsWith('image') ? 'image' : 'video'}`;
        ...
        const handleUrlInsert = (type) => {
            ...
            if (url) {
                ...
                media.src = url;
                media.className = `mt-editor-${type}`;
                if (media.tagName === 'VIDEO') media.controls = true;
                insertElementAtCursor(media);
                editor.focus();
            }
